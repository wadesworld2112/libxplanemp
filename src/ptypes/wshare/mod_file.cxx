...
void file_req::rsp_dir_index()
{
    ...
    if (req->method == HTTP_GET)     // not HEAD
...
void file_req::rsp_file()
{
    ...
    {
        if (req->range_max == -1)
            req->range_max = fsize - 1;
        if (req->range_min >= 0 && req->range_min < req->range_max && req->range_max < fsize)
    ...
    if (req->partial)
        req->put_header("Content-Range",
            "bytes " + itostring(req->range_min) + '-' + itostring(req->range_max) + '/' + itostring(fsize));
    if (cfg_file_mtime)
    ...
    // send content
    if (req->partial)
        f.seek(req->range_min); // will implement 64-bit seek in the future
