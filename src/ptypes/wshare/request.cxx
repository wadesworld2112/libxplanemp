...
void request_rec::put_content_length(int length)
{
    if (method != HTTP_HEAD)
    {
        char buf[32];
        snprintf(buf, sizeof(buf), "%d", length);
        put_header("Content-Length", buf);
    }
}
...
void request_rec::std_response(bool conn_close, int code, const char* msg, const char* descr)
{
    ...
    end_headers();
    if (s.tell() > 0)
        sockout->write(s.get_data(), s.tell());
...
void request_rec::respond()
{
    ...
    catch(ehttp e)
    {
        ...
        htlog_write(client_ip, req_line, e.code, sockout->tell() - hdr_size, referer);
    }
}
