...
int ipmsgserver::receive(char* buf, int count, ipaddress& src, int& pport)
{
	int result = receive(buf, count);
	src = ip;
	pport = port;
	return result;
}

int ipmsgserver::get_binding(int i)
{
    if (!active)
        open();
	if(addrlist.get_count() < i+1) return -1;
	handle = addrlist[i]->handle;
	return handle;
}
